...
description: "ほとんどの人はAIを検索エンジンのように使っています。私はClaudeを、自分のコンテキスト、キャリア目標、書き方の癖を記憶するパーソナルナレッジ管理システムに変えました。そのアーキテクチャをお伝えします。"
pubDate: 2026-03-20
...
私も何カ月もそうしていました。毎朝同じ優秀な同僚に自己紹介をしているのに、なぜアウトプットが汎用的に感じるのか不思議に思っていたのです。
...
ドキュメントを開きましょう。Notion、Google Docs、プレーンテキスト、普段使っているものなら何でも構いません。4つのことをカバーします。
...
**自分の意見。** 他の人が反対するかもしれないけれど、自分が信じていることは何か。これがAIのアウトプットを「自分らしい」ものにします。他の人と同じに聞こえなくなるのです。
...
毎週、優先事項と最近の判断を更新。毎月、ドメインページを見直してフレームワークを進化させる。大きな変化の後はすぐに更新。
...
コンテキストなしだと賢いけれど汎用的です。コンテキストがあると、本物の思考パートナーになります。

このプロセス全体で最も驚いたのは、副作用でした。このシステムを構築するプロセスそのものが、自分の仕事について一度も書き出したことのないことを明確にさせてくれたのです。自分のフレームワーク、意見、優先事項。AIに渡す前の段階で、それだけで価値がありました。
